...
const base={...dom,console,setTimeout,clearTimeout,AbortController,Uint8Array,ArrayBuffer,URL,Date,crypto:require('node:crypto').webcrypto,
 esc:escape,appleEsc:escape,tekstDoPorownania:s=>String(s||'').trim().toLowerCase(),smartTypy:new WeakMap(),smartAbortController:null,smartOcrWorker:null,_stanImportu:null,
...
test('OCR używa lokalnego workera zgodnego z CSP',async()=>{
 let options;const c=context({smartWczytajSkrypt:async()=>{},Tesseract:{createWorker:async(lang,oem,opt)=>{assert.equal(lang,'pol');options=opt;return {terminate:async()=>{}};}}});
 await c.smartWorkerOcr();assert.equal(options.workerBlobURL,false);assert.equal(new URL(options.workerPath).origin,'https://example.test');await c.smartZakonczOcr();assert(html.includes("worker-src 'self';"));
});
